...
2. **For each phase in `structure.md`**, expand into full implementation detail:
   ...
   - Code snippets for non-trivial changes (new functions, type definitions, decoders)
...
3. **Write `plan.md`** to the artifact directory:
   ...
   **File**: `internal/beanfun/thing.go`
...
- File written: `docs/plans/<id>/plan.md`
- Tell the user: "Next: run `/qrspi:6_worktree docs/plans/<id>/` to set up an isolated worktree, or `/qrspi:7_implement docs/plans/<id>/` to implement in the current tree."
...
- Verification commands come from this project's toolchain: `gofmt`, `golangci-lint run`, `go build ./...`, `go test ./internal/...`, and `task dev` / `task build` / `task windows:build`. Check `CLAUDE.md` and `Taskfile.yml` before inventing one.
- Tests are table-driven. HTTP is faked with `httptest`; keyring is mocked. Every new behaviour needs at least one assertion that pins a literal expected value, not just internal consistency.
...
- If a bound service method's signature changes, the plan must include regenerating the Wails bindings under `frontend/bindings/` (running `task dev` regenerates them) and committing the result. Note the fallback: if generation fails, hand-edit the binding and flag it.
- If a phase touches Windows-only code, the plan must say the verification is deferred to a Windows session rather than listing a command that will silently not run.
